...
- [x] 補齊無素材產文、多平台 target／覆寫、格式驗證 API、AI／輸入錯誤、時區／夏令時間、失敗通知、JSON 儲存層、平台節流、錯誤記錄、系統健康、部署前置與單一操作員登入測試；完整測試目前 160/160 通過。
