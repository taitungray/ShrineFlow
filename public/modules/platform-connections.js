...
export function renderPlatformConnections() {
  ...
  const cards = platforms.map((platform) => {
    ...
    return '<article class="platform-connection-card" data-status="' + status + '">'
      ...
      + '<div class="platform-connection-footer"><span class="helper">' + (healthNotice || (accounts.length ? '目前品牌：' + accounts.length + ' 個平台連線' : '目前品牌尚未設定')) + '</span><a class="btn-text" href="#/platforms/' + escapeHtml(platform.id) + '">' + (configured ? '管理連線 →' : '前往連線 →') + '</a></div>'
      + '</article>';
  });
  if (summary) {
