...
function messageFor(result) {
  if (!result || result.status === 'insufficient_data') {
    return '目前樣本不足，先累積至少 ' + escapeHtml(String(result?.minimumSamples || 10)) + ' 筆已發布紀錄。';
  }
...
export function renderBestTimes() {
  ...
  const filterText = result.filters?.platformId
    ? PLATFORM_NAMES[result.filters.platformId] || result.filters.platformId
    : '全部平台';
  ...
  container.innerHTML = '<div class="best-times-heading"><div><span class="section-tag">TIMING SUGGESTION</span><h3>最佳發布時段</h3></div><button class="btn-test" id="refreshBestTimesButton" type="button">分析</button></div>'
    + '<p class="helper">' + messageFor(result) + '</p>'
    + '<p class="best-times-meta">' + escapeHtml(filterText) + ' · ' + escapeHtml(result.timeZone || 'Asia/Taipei') + ' · ' + escapeHtml(String(result.sampleCount || 0)) + ' 筆樣本 · ' + escapeHtml(result.algorithmVersion || '—') + '</p>'
    + slotMarkup;
...
export async function loadBestTimes() {
  ...
  try {
    state.bestTimes = await api(clientQuery('/api/insights/best-times'));
    renderBestTimes();
  } catch (error) {
