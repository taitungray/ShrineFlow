...
8. 若使用 Inbox webhook 同步，設定 `META_APP_SECRET` 與 `META_WEBHOOK_VERIFY_TOKEN`；readiness 會檢查兩者是否同時存在。
...
- 主要集合有硬上限：品牌 100 筆／單品牌 20 個平台連線、內容 5,000 筆、模板／活動各 500 筆；主要 JSON 另有 bytes 上限，達上限時停止新增，不自動刪除內容。
