...
import { HISTORY_RETENTION_POLICIES } from './history-retention.js';
import { getRepositories } from './repositories.js';
import { getMediaStorage } from './media-storage.js';
import { cleanupOrphanMedia } from './cloud-backup.js';
...
export const UPLOAD_RETENTION_POLICY = Object.freeze({
  orphanMaxAgeDays: 7,
  maxFileCount: 1000,
...
export async function createBackup({ includeMedia = false, preserveIds = [] } = {}) {
  ...
  for (const directoryName of ['insights', 'publishAttempts', 'postVersions']) {
...
export async function getUploadQuotaStatus({ incomingBytes = 0, incomingFiles = 0 } = {}) {
  const files = getMediaStorage().backend === 'r2'
    ? (await getRepositories().mediaAssets.list())
      .filter((asset) => asset?.status === 'ready' && !asset.deletedAt)
      .map((asset) => ({
        name: asset.objectKey,
        size: Number(asset.sizeBytes) || 0,
        modifiedAt: asset.updatedAt || asset.createdAt,
      }))
    : await uploadFiles();
  const totalBytes = files.reduce((sum, file) => sum + file.size, 0);
  const projectedBytes = totalBytes + Math.max(0, Number(incomingBytes) || 0);
  const projectedFileCount = files.length + Math.max(0, Number(incomingFiles) || 0);
  const reasons = [];
  if (projectedBytes > UPLOAD_RETENTION_POLICY.maxTotalBytes) reasons.push('total_bytes');
  if (projectedFileCount > UPLOAD_RETENTION_POLICY.maxFileCount) reasons.push('file_count');
  return {
    ...
    projectedFileCount,
    projectedBytes,
    policy: UPLOAD_RETENTION_POLICY,
  };
}
...
async function inspectHistoryDirectory(directory, policy) {
  ...
  return {
    ...
    recordCount,
    totalBytes,
    oldestArchive: names[0] || null,
    newestArchive: names[names.length - 1] || null,
    policy,
  };
}

async function scanR2StorageHealth() {
  ...
  for (const post of posts) {
    ...
    ]) referenced.add(String(mediaPath));
  }
  const assets = (await repositories.mediaAssets.list()).filter((asset) => asset?.status === 'ready' && !asset.deletedAt);
  const orphanAssets = assets.filter((asset) => !referenced.has(String(asset.mediaPath)));
  const orphanCutoff = Date.now() - (UPLOAD_RETENTION_POLICY.orphanMaxAgeDays * 24 * 60 * 60 * 1000);
  const eligibleOrphanAssets = orphanAssets.filter((asset) => Date.parse(asset.createdAt || asset.updatedAt || '') < orphanCutoff);
  const totalBytes = assets.reduce((sum, asset) => sum + (Number(asset.sizeBytes) || 0), 0);
  const orphanBytes = orphanAssets.reduce((sum, asset) => sum + (Number(asset.sizeBytes) || 0), 0);
  const eligibleOrphanBytes = eligibleOrphanAssets.reduce((sum, asset) => sum + (Number(asset.sizeBytes) || 0), 0);
  return {
    ...
    uploads: {
      ...
      totalBytes,
      referencedFileCount: assets.length - orphanAssets.length,
      orphanFileCount: orphanAssets.length,
      orphanBytes,
      orphanFiles: orphanAssets.map((asset) => ({ name: asset.objectKey, size: Number(asset.sizeBytes) || 0, modifiedAt: asset.updatedAt || asset.createdAt })),
      eligibleOrphanFileCount: eligibleOrphanAssets.length,
      eligibleOrphanBytes,
      eligibleOrphanFiles: eligibleOrphanAssets.map((asset) => ({ name: asset.objectKey, size: Number(asset.sizeBytes) || 0, modifiedAt: asset.updatedAt || asset.createdAt })),
    },
    backups: [],
    history: { provider: 'firestore' },
    policy: {
      ...
      historyRetention: HISTORY_RETENTION_POLICIES,
      uploadRetention: UPLOAD_RETENTION_POLICY,
    },
  };
}
export async function scanStorageHealth() {
  if (getMediaStorage().backend === 'r2') return scanR2StorageHealth();
  ...
  const orphanCutoff = Date.now() - (UPLOAD_RETENTION_POLICY.orphanMaxAgeDays * 24 * 60 * 60 * 1000);
  const eligibleOrphanFiles = orphanFiles.filter((file) => Date.parse(file.modifiedAt) < orphanCutoff);
  const totalBytes = files.reduce((sum, file) => sum + file.size, 0);
  const orphanBytes = orphanFiles.reduce((sum, file) => sum + file.size, 0);
  const eligibleOrphanBytes = eligibleOrphanFiles.reduce((sum, file) => sum + file.size, 0);
  return {
    generatedAt: new Date().toISOString(),
    uploads: {
      fileCount: files.length,
      ...
      orphanBytes,
      orphanFiles,
      eligibleOrphanFileCount: eligibleOrphanFiles.length,
      eligibleOrphanBytes,
      eligibleOrphanFiles,
    },
    backups: await listBackups(),
    history: {
      postVersions: await inspectHistoryDirectory(directories.postVersions, HISTORY_RETENTION_POLICIES.postVersions),
      publishAttempts: await inspectHistoryDirectory(directories.publishAttempts, HISTORY_RETENTION_POLICIES.publishAttempts),
      insights: await inspectHistoryDirectory(directories.insights, HISTORY_RETENTION_POLICIES.insights),
    },
    policy: {
      secretsIncluded: false,
      orphanDeletionRequiresExplicitConfirmation: true,
...
export async function cleanupOrphanUploads({ confirm = false, mode = 'manual' } = {}) {
  if (getMediaStorage().backend === 'r2') {
    const health = await scanStorageHealth();
    if (!confirm && mode !== 'automatic') return { dryRun: true, deleted: [], health };
    const result = await cleanupOrphanMedia({ repositories: getRepositories() });
    return { dryRun: false, mode, deleted: result.deleted, health: await scanStorageHealth() };
  }
  const health = await scanStorageHealth();
  const filesToDelete = mode === 'automatic'
    ? health.uploads.eligibleOrphanFiles
    : health.uploads.orphanFiles;
  if (!confirm && mode !== 'automatic') return { dryRun: true, deleted: [], health };
  const deleted = [];
  ...
  return { dryRun: false, mode, deleted, health: await scanStorageHealth() };
}
