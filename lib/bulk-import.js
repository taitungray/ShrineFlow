...
const VIDEO_METADATA_WARNING_CODES = new Set([
  'media_kind_unverified',
  'video_metadata_unverified',
  'video_duration_unverified',
  'video_dimensions_unverified',
]);
...
export async function validateBulkCsv(input, {
  ...
  for (const entry of parsed.rows) {
    ...
    const videoTarget = ['reel', 'story'].includes(contentType)
      || mediaPaths.some((mediaPath) => /\.(avi|m4v|mov|mp4|mpeg|mpg|ogv|webm)(?:[?#].*)?$/i.test(String(mediaPath)));
    if (requireVideoMetadata && videoTarget) {
      const unverified = validation.warnings.filter((warning) => VIDEO_METADATA_WARNING_CODES.has(warning.code));
      if (unverified.length) rowErrors.push(issue('VIDEO_METADATA_REQUIRED', '影片缺少可驗證的類型、長度或尺寸資訊，已停止匯入。', 'mediaPaths'));
    }
    rows.push({
