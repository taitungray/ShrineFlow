...
export function getSecretMasterKey(explicitKey = undefined) {
  return String(explicitKey === undefined ? process.env.SHRINEFLOW_MASTER_KEY || '' : explicitKey).trim();
}
...
export function encryptSecret(value, explicitKey = undefined) {
...
export function decryptSecret(value, explicitKey = undefined) {
...
export function decryptClientSecrets(client, explicitKey = undefined) {
...
export function encryptClientSecrets(client, explicitKey = undefined) {
  const next = clone(client);
  for (const field of SECRET_FIELDS) {
