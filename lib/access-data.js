...
export function assertInvitationEmailAllowed(email, env = process.env) {
  const domain = emailDomain(email);
  const allowed = domainSet(env.SHRINEFLOW_ALLOWED_EMAIL_DOMAINS);
  const blocked = domainSet(env.SHRINEFLOW_BLOCKED_EMAIL_DOMAINS);
  if (blocked.has(domain)) {
    const error = new Error('Invitation email domain is blocked.');
    error.code = 'INVITATION_DOMAIN_BLOCKED';
    error.status = 403;
    throw error;
  }
  if (allowed.size && !allowed.has(domain)) {
    const error = new Error('Invitation email domain is not allowed.');
    error.code = 'INVITATION_DOMAIN_NOT_ALLOWED';
    error.status = 403;
    throw error;
  }
  return domain;
...
function activeMembershipsByCreatedAt(records = []) {
  return records
...
export async function listMembershipsForUser(userId, repositories = getRepositories()) {
  const records = await repositories.memberships.query({
    filters: { userId: String(userId || '').trim() },
  });
  return activeMembershipsByCreatedAt(records);
}

export async function listMembershipsForClient(clientId, repositories = getRepositories()) {
  const records = await repositories.memberships.query({
    filters: { clientId: String(clientId || '').trim() },
  });
  return activeMembershipsByCreatedAt(records);
}
...
export async function createInvitation(input = {}, repositories = getRepositories(), {
  ...
  assertInvitationEmailAllowed(emailNormalized, env);
...
export async function ensureSystemOwnerMemberships(user, repositories = getRepositories()) {
  const uid = String(user?.uid || user?.id || '').trim();
  if (!uid) return [];
  const existing = await listMembershipsForUser(uid, repositories);
  const have = new Set(existing.map((membership) => membership.clientId));
  const memberships = [...existing];
  const clients = await repositories.clients.list();
  for (const client of clients) {
    if (!client?.id || have.has(client.id)) continue;
    memberships.push(await upsertMembership({
      clientId: client.id,
      userId: uid,
      role: 'owner',
...
export async function bootstrapOwner(user, repositories = getRepositories()) {
  ...
  const memberships = await ensureSystemOwnerMemberships(savedUser, repositories);
