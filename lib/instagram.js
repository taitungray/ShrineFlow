...
export function createInstagramPublisher({
  ...
  async function graphRequest(resource, options = {}) {
    ...
    try {
      ...
      if (error?.code === 'PROVIDER_RATE_LIMIT_LOCAL') {
        throw new InstagramPublishError(error.message, { code: error.code, retriable: true });
      }
      throw new InstagramPublishError(
  ...
  async function publishFirstComment({ mediaId, text } = {}) {
    assertConfigured();
    const normalizedMediaId = String(mediaId || '').trim();
    const message = String(text || '').trim();
    if (!normalizedMediaId) throw new InstagramPublishError('Instagram 首則留言缺少已發布媒體 ID。');
    if (!message) throw new InstagramPublishError('Instagram 首則留言不可為空白。');
    const payload = await graphRequest(`${normalizedMediaId}/comments`, {
      body: formBody({ message }),
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    });
    if (!payload.id) throw new InstagramPublishError('Instagram 未回傳首則留言 ID。');
  ...
  return { configured, publish, publishFirstComment, verify };
}
