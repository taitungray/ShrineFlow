...
export function createAuthRouter({
  ...
  router.get('/auth/status', async (request, response) => {
    const stat = await authService.status(readCookie(request, AUTH_COOKIE_NAME));
    response.json({ version: getAppVersion(), ...stat });
  });
  ...
  async function logout(request, response) {
    const session = await authService.authenticate(readCookie(request, AUTH_COOKIE_NAME));
    const actor = session?.actor || (session?.authenticated ? createLegacyOwnerActor() : null);
    if (actor) {
      await authService.recordSecurity?.({
        type: 'logout',
        actor,
