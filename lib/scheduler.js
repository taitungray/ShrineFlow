...
export async function reconcilePassedNativeSchedules({ repositories = getRepositories(), now = new Date() } = {}) {
  const currentTime = now instanceof Date ? now.getTime() : new Date(now).getTime();
  return repositories.posts.mutate((posts) => {
    let mutated = false;
    for (const post of posts) {
      if (isPostArchived(post)) continue;
      const normalized = migrateLegacyPost(post, post.clientId || '');
      let changed = false;
      for (const target of normalized.targets || []) {
        if (
          ...
          target.scheduledAt &&
          new Date(target.scheduledAt).getTime() <= currentTime &&
          (target.platformId === 'facebook' || target.externalId)
        ) {
          target.status = 'published';
          target.publishedAt = target.publishedAt || target.scheduledAt;
          ...
          if (Array.isArray(target.publishAttempts) && target.publishAttempts.length > 0) {
            ...
            if (lastAttempt && lastAttempt.status === 'started') {
              ...
              lastAttempt.externalId = target.externalId || lastAttempt.externalId;
            }
          }
          if (target.platformId === 'facebook' && target.externalId) {
            post.facebookPostId = target.externalId;
          }
          post.publishedAt = post.publishedAt || target.publishedAt;
          changed = true;
          mutated = true;
        }
      }
      if (changed) {
        ...
        post.status = summarizePostStatus(normalized.targets);
        bumpPostVersion(post);
      }
    }
    return mutated;
...
export function createScheduler({
  ...
  async function processDueSchedules(now = new Date()) {
    ...
    try {
      await reconcilePassedNativeSchedules({ repositories, now });
