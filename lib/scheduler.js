...
export function shouldClaimTargetForLocalPublish(target, now = new Date()) {
  if (target.status === 'publishing') {
    return Boolean(target.leaseExpiresAt && new Date(target.leaseExpiresAt) <= now);
  }
...
export function createScheduler({
  ...
  async function finishTarget(claim, result) {
    ...
    if (finalizedPost) await appendPostVersion({ post: finalizedPost, source: 'publish', actor: 'scheduler' });
    return publishedAt;
  }
