...
import { isPostArchived, isPostHidden, isPostIdea } from '../post-lifecycle.js';
...
import { reconcilePassedNativeSchedules } from '../scheduler.js';
...
const BLOCKED_SCHEDULE_STATUSES = new Set(['scheduled', 'publishing', 'pending']);

function scheduleHttpError(message, status, code) {
...
function buildScheduleItem(post, storedTarget) {
  return {
    id: storedTarget.id,
    postId: post.id,
    targetId: storedTarget.id,
    clientId: post.clientId,
    channel: storedTarget.platformId,
    accountId: storedTarget.accountId,
    contentType: storedTarget.contentType,
    contentSettings: storedTarget.contentSettings,
    timeZone: storedTarget.timeZone || null,
    scheduledAt: storedTarget.scheduledAt,
    status: storedTarget.status,
    externalId: storedTarget.externalId || null,
...
function queueFieldsFromAssignment(queueAssignment) {
...
async function revertPublishingScheduleClaim({ repositories, postId, targetId, error }) {
  try {
    await repositories.posts.mutate((storedPosts) => {
      ...
      if (!storedPost || !storedTarget || storedTarget.status !== 'publishing') return null;
      Object.assign(storedTarget, {
        status: 'failed',
        scheduledAt: null,
        externalId: null,
        lastError: {
          message: error?.message || '排程失敗。',
          code: error?.code || '',
          at: new Date().toISOString(),
        },
      });
      delete storedTarget.publishingStartedAt;
      delete storedTarget.leaseId;
      delete storedTarget.leaseExpiresAt;
      storedPost.status = summarizePostStatus(storedPost.targets);
      bumpPostVersion(storedPost);
      return storedTarget;
    });
  } catch {
    // Revert must not hide the original schedule error.
  }
}
...
export function createScheduleRouter({ publishingPlatforms, resolveFacebookPublisher, repositories = getRepositories() }) {
  ...
  router.post('/schedule', async (request, response) => {
    ...
    const queueFields = queueFieldsFromAssignment(queueAssignment);
    const idempotencyKey = String(request.get('Idempotency-Key') || request.body?.idempotencyKey || '').trim();
    let claim;
    try {
      claim = await repositories.posts.mutate((storedPosts) => {
        const storedPost = storedPosts.find((item) => item.id === postId);
        if (!storedPost) throw scheduleHttpError('找不到要排程的貼文。', 404, 'POST_NOT_FOUND');
        ...
        let storedTarget = storedNormalized.targets.find((item) => item.id === target.id)
          || storedNormalized.targets.find((item) => (
            item.accountId === target.accountId && item.platformId === channel && item.contentType === contentType
          ));
        if (!storedTarget && !storedPost.targets?.length && storedNormalized.targets.length === 1) {
          storedTarget = storedNormalized.targets[0];
          if (accountId) storedTarget.accountId = accountId;
        }
        if (!storedTarget) {
        ...
        if (idempotencyKey
          && storedTarget.scheduleIdempotencyKey === idempotencyKey
          && storedTarget.status === 'scheduled') {
          Object.assign(storedPost, storedNormalized);
          return {
            replay: true,
            post: storedPost,
            target: storedTarget,
            version: Number(storedPost.version || 1),
            item: buildScheduleItem(storedPost, storedTarget),
          };
        }
        if (storedTarget.status === 'published') {
          throw scheduleHttpError('此平台目標已發布，請建立副本後再重新排程。', 409, 'ALREADY_PUBLISHED');
        }
        const stalePublishing = storedTarget.status === 'publishing' && isStalePublishingLock(storedTarget);
        if (BLOCKED_SCHEDULE_STATUSES.has(storedTarget.status) && !stalePublishing) {
          throw scheduleHttpError(
            storedTarget.status === 'scheduled'
              ? '此平台已排程，請改時間或先取消，勿重複送出。'
              : '此平台目標目前正在排程或發布中，請稍後再試。',
            409,
            storedTarget.status === 'scheduled' ? 'ALREADY_SCHEDULED' : 'SCHEDULE_IN_PROGRESS',
          );
        }
        ...
        Object.assign(storedTarget, {
          ...
          lastError: null,
          scheduleIdempotencyKey: idempotencyKey || storedTarget.scheduleIdempotencyKey || null,
          ...queueFields,
        });
        delete storedTarget.nextAttemptAt;
        storedNormalized.status = summarizePostStatus(storedNormalized.targets);
        bumpPostVersion(storedNormalized);
        Object.assign(storedPost, storedNormalized);
        return {
          replay: false,
          post: storedPost,
      ...
      return response.status(error.status || 500).json({ error: error.message, code: error.code });
    }
    if (claim.replay) {
      return response.status(201).json({ ...claim.item, replayed: true });
    }
    target = claim.target;
    ...
    if (channel === 'facebook') {
      try {
        ...
        result = await scheduleFacebookTarget({
          publisher,
          post: normalized,
        ...
        await revertPublishingScheduleClaim({
          repositories,
          postId,
          targetId: target.id,
          error,
    ...
    try {
      ...
      const storedNormalized = migrateLegacyPost(storedPost, storedPost.clientId || normalized.clientId);
      if (Number(storedNormalized.version || 1) !== claim.version) {
        return {
    ...
    if (target.platformId === 'facebook') {
      ...
      try {
        ...
        await appendErrorLog({
          ...
          error,
          platformId: 'facebook',
          status: error.remoteDeleted ? 502 : 400,
        });
