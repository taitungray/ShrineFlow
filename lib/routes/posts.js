...
import { addPostLifecycleEvent, activePostTargetStatuses, isPostArchived } from '../post-lifecycle.js';
...
export function createPostsRouter({ repositories = getRepositories() } = {}) {
  ...
  router.post('/posts/:postId/duplicate', async (request, response) => {
    ...
    const clientId = String(request.body?.clientId || sourcePost.clientId || fallbackClientId).trim();
