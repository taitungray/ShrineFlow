...
export function createPostsRouter({ repositories = getRepositories() } = {}) {
  ...
  router.post('/posts/:postId/mark-published', async (request, response) => {
    ...
    const updated = await repositories.posts.mutate((posts) => {
      ...
      for (const target of current.targets || []) {
        ...
        target.publishedAt = target.publishedAt || target.scheduledAt || publishedAt;
        target.lastError = null;
        if (Array.isArray(target.publishAttempts) && target.publishAttempts.length > 0) {
          const lastAttempt = target.publishAttempts[target.publishAttempts.length - 1];
          if (lastAttempt && lastAttempt.status === 'started') {
            lastAttempt.status = 'succeeded';
            lastAttempt.finishedAt = target.publishedAt;
          }
        }
      }
      current.status = summarizePostStatus(current.targets);
      current.publishedAt = current.publishedAt || publishedAt;
      if (current.targets?.[0]?.externalId) {
        current.facebookPostId = current.targets[0].externalId;
      }
      bumpPostVersion(current);
      addPostLifecycleEvent(current, 'published', { manual: true }, publishedAt);
      posts[index] = current;
      return { kind: 'updated', post: presentPost(current) };
    });
    if (updated.kind === 'not_found') return response.status(404).json({ error: '找不到要更新的貼文。' });
    await appendPostVersion({ post: updated.post, source: 'publish', actor: request.actor?.email || 'manual' });
    response.json({ ...updated.post, lifecycleAction: 'published' });
  });
