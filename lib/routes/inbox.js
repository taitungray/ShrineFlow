...
export function createInboxRouter({
  ...
  router.patch('/inbox/items/:itemId', async (request, response) => {
    try {
      const metadata = await updateInboxItemMetadata({
        ...
        ...(request.body?.needsReply === undefined ? {} : { needsReply: request.body.needsReply }),
  ...
  router.get('/inbox', async (request, response) => {
    ...
    const needsReplyOnly = request.query.needsReplyOnly === 'true';
    ...
    const sources = await Promise.all(accounts.map(async (account) => {
      ...
      try {
        ...
        const providerItems = await applyInboxItemMetadata(payload.items, base);
        const items = providerItems.filter((item) => (
          (!unreadOnly || item.unread)
          && (!needsReplyOnly || item.needsReply)
        ));
        ...
        return {
          ...
          filteredOutCount: providerItems.length - items.length,
          filter: { unreadOnly, needsReplyOnly },
          status: 'synced',
