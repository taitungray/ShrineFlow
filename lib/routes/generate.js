...
export function createGenerateRouter({
  ...
  async function respondSettledJob(response, jobId, runner) {
    await settleJob(jobId, runner);
    const finished = jobsOf().get(jobId);
    if (finished?.status === 'succeeded') {
      return response.status(200).json(finished.result);
    }
    const error = finished?.error || serializeJobError(new Error('背景工作失敗。'));
    return response.status(error.status || 500).json({
      error: error.message || '背景工作失敗。',
      code: error.code || '',
    });
  }
  ...
  router.post('/generate', enforceUploadQuota, upload.array('media'), enforceUploadedFileQuota, async (request, response) => {
    ...
    await respondSettledJob(response, job.id, () => runGenerateJob(job.id));
  });
  ...
  router.post('/rewrite', async (request, response) => {
    ...
    await respondSettledJob(response, job.id, () => runRewriteJob(job.id));
  });
