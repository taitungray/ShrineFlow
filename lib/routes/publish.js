import { Router } from 'express';
import { bumpPostVersion } from '../post-version.js';
import { getRepositories } from '../repositories.js';
import { getMediaStorage } from '../media-storage.js';
import { resolvePostMediaPaths } from '../upload.js';
import { normalizePostCopy } from '../copy-format.js';
import { createFacebookPublisher, FacebookPublishError } from '../facebook.js';
import { InstagramPublishError } from '../instagram.js';
import { ThreadsPublishError } from '../threads.js';
import { getClientRaw, findAccount } from '../clients.js';
import {
  migrateLegacyPost,
  resolveTargetCopy,
  resolveTargetMedia,
  summarizePostStatus,
} from '../post-targets.js';
import {
  appendPublishAttempt,
  createPublishAttempt,
  serializePublishError,
  updatePublishAttempt,
} from '../publish-reliability.js';
import {
  findPublishAttemptByIdempotencyKey,
  recordPublishAttemptEvent,
} from '../publish-attempt-log.js';
import { directories } from '../store.js';
import { formatValidationError, validateTargetFormat } from '../content-validation.js';
import { appendPostVersion } from '../post-history.js';
import { isPostArchived, isPostIdea } from '../post-lifecycle.js';
import { isStalePublishingLock } from '../publish-lock.js';
import { approvalGate } from '../approval-workflow.js';

function mediaFilePathsFromWebPaths(mediaPaths = []) {
  return mediaPaths
    .filter((mediaPath) => String(mediaPath).startsWith('/uploads/'))
    .map((mediaPath) => getMediaStorage().resolveFilePath(mediaPath))
    .filter(Boolean);
}

async function mediaBuffersFromWebPaths(mediaPaths = []) {
  const storage = getMediaStorage();
  if (storage.backend !== 'r2') return [];
  return Promise.all(mediaPaths.map(async (mediaPath) => ({
    path: mediaPath,
    name: String(mediaPath).split('/').pop() || 'media',
    buffer: await storage.getBuffer(mediaPath),
  })));
}

function unavailablePublisherError(platformId) {
  const names = {
    facebook: 'Facebook',
    instagram: 'Instagram',
    threads: 'Threads',
  };
  const error = new Error(`${names[platformId] || platformId} 尚未設定。請先填入平台發布憑證。`);
  error.status = 503;
  return error;
}

function isPublicMediaConfigurationError(error) {
  return (
    error instanceof InstagramPublishError
    || error instanceof ThreadsPublishError
  ) && /PUBLIC_MEDIA_BASE_URL|公開網址/i.test(String(error.message || ''));
}

function publishConflict(message) {
  const error = new Error(message);
  error.status = 409;
  return error;
}

function remotePublishReconciliationError(externalId, reason = '') {
  const error = new Error('平台已回應發布成功，但本地資料尚未完成同步，請先確認平台結果。');
  error.status = 409;
  error.code = 'REMOTE_PUBLISH_RECONCILIATION_REQUIRED';
  error.externalId = externalId || null;
  error.reason = reason;
  return error;
}

function firstCommentFailure(error) {
  return serializePublishError(error || new Error('Instagram 首則留言失敗。'));
}

async function saveFirstCommentState({ postId, targetId, patch, repositories = getRepositories() }) {
  return repositories.posts.mutate((records) => {
    const post = records.find((item) => item.id === postId);
    if (!post) return null;
    const migrated = migrateLegacyPost(post, post.clientId || '');
    Object.assign(post, migrated);
    const target = post.targets?.find((item) => item.id === targetId);
    if (!target) return null;
    target.delivery = target.delivery || {};
    target.delivery.firstComment = {
      ...(target.delivery.firstComment || {}),
      ...patch,
    };
    bumpPostVersion(post);
    return { post, target: { ...target } };
  });
}

async function publishFirstComment({
  postId,
  targetId,
  text,
  publisher,
  repositories = getRepositories(),
}) {
  const posts = await repositories.posts.list();
  const rawPost = posts.find((item) => item.id === postId);
  if (!rawPost) {
    const error = new Error('找不到要留言的貼文。');
    error.status = 404;
    throw error;
  }
  const post = migrateLegacyPost(rawPost, rawPost.clientId || '');
  const target = post.targets?.find((item) => item.id === targetId);
  if (!target) {
    const error = new Error('找不到要留言的平台目標。');
    error.status = 404;
    throw error;
  }
  if (target.platformId !== 'instagram') {
    const error = new Error('目前只有 Instagram Professional target 支援首則留言。');
    error.status = 400;
    error.code = 'FIRST_COMMENT_UNSUPPORTED_PLATFORM';
    throw error;
  }
  if (target.status !== 'published' || !target.externalId) {
    const error = new Error('主貼文發布成功並取得平台 ID 後，才能發布首則留言。');
    error.status = 409;
    error.code = 'FIRST_COMMENT_MAIN_POST_NOT_READY';
    throw error;
  }
  const existing = target.delivery?.firstComment || {};
  if (existing.status === 'published' && existing.externalId && !String(text || '').trim()) {
    return { targetId, firstComment: existing, replayed: true };
  }
  const message = String(text || existing.text || '').trim().slice(0, 2000);
  if (!message) {
    const error = new Error('請輸入首則留言內容。');
    error.status = 400;
    throw error;
  }

  await saveFirstCommentState({
    postId,
    targetId,
    patch: { status: 'pending', text: message, lastError: null },
    repositories,
  });

  let result;
  try {
    if (!publisher?.configured) throw unavailablePublisherError('instagram');
    if (typeof publisher.publishFirstComment !== 'function') {
      const error = new Error('此 Instagram 連線尚未通過首則留言 capability 驗證。');
      error.status = 409;
      error.code = 'FIRST_COMMENT_CAPABILITY_UNAVAILABLE';
      throw error;
    }
    result = await publisher.publishFirstComment({ mediaId: target.externalId, text: message });
  } catch (error) {
    const failure = firstCommentFailure(error);
    await saveFirstCommentState({
      postId,
      targetId,
      patch: { status: 'failed', text: message, lastError: failure },
      repositories,
    });
    const responseError = error instanceof Error ? error : new Error(failure.message);
    responseError.status = responseError.status || 502;
    responseError.code = responseError.code || 'FIRST_COMMENT_FAILED';
    responseError.firstComment = { status: 'failed', text: message, lastError: failure };
    throw responseError;
  }

  const publishedAt = new Date().toISOString();
  const saved = await saveFirstCommentState({
    postId,
    targetId,
    patch: {
      status: 'published',
      text: message,
      externalId: result?.externalId || null,
      lastError: null,
      publishedAt,
    },
    repositories,
  });
  if (saved?.post) await appendPostVersion({ post: saved.post, source: 'first_comment' });
  return { targetId, firstComment: saved?.target?.delivery?.firstComment || { status: 'published', text: message, externalId: result?.externalId || null }, replayed: false };
}

async function beginPublishAttempt({ postId, targetId, source, idempotencyKey, repositories = getRepositories() }) {
  const archivedAttempt = await findPublishAttemptByIdempotencyKey({
    postId,
    targetId,
    idempotencyKey,
  });
  return repositories.posts.mutate((records) => {
    const postRaw = records.find((item) => item.id === postId);
    if (!postRaw) {
      const error = new Error('找不到要發布的內容。');
      error.status = 404;
      throw error;
    }

    const post = migrateLegacyPost(postRaw, postRaw.clientId || '');
    if (isPostArchived(post)) throw publishConflict('封存中的貼文不能發布，請先還原。');
    const target = (post.targets || []).find((item) => item.id === targetId)
      || (post.targets?.length === 1 ? post.targets[0] : null);
    if (!target) {
      const error = new Error('找不到要發布的平台目標。');
      error.status = 400;
      throw error;
    }

    const existingAttempt = idempotencyKey
      ? (target.publishAttempts || []).find((item) => item.idempotencyKey === idempotencyKey) || archivedAttempt
      : null;
    if (existingAttempt) {
      if (
        existingAttempt.status === 'succeeded'
        && target.status === 'published'
        && (!existingAttempt.externalId || target.externalId === existingAttempt.externalId)
      ) {
        return { replay: true, post, target, attempt: existingAttempt };
      }
      throw publishConflict('此發布請求已經處理過，請使用新的請求識別碼。');
    }

    if (target.lastError?.code === 'REMOTE_PUBLISH_RECONCILIATION_REQUIRED') {
      throw publishConflict('平台發布結果尚未完成對帳，請先確認平台狀態後再處理。');
    }
    if (target.status === 'published') {
      throw publishConflict('此平台目標已發布，請建立副本後再重新發布。');
    }
    if (['scheduled', 'pending'].includes(target.status)
      || (target.status === 'publishing' && !isStalePublishingLock(target))) {
      throw publishConflict('此平台目標目前正在排程或發布中，請先完成目前流程。');
    }

    const attempt = createPublishAttempt({ source, idempotencyKey });
    target.status = 'publishing';
    target.publishingStartedAt = attempt.startedAt;
    target.attempts = Number(target.attempts || 0) + 1;
    target.lastAttemptAt = attempt.startedAt;
    target.lastError = null;
    appendPublishAttempt(target, attempt);
    post.status = summarizePostStatus(post.targets);
    bumpPostVersion(post);
    Object.assign(postRaw, post);
    return { replay: false, post, target, attempt };
  });
}

async function resolvePublisher({
  post,
  target,
  account,
  fallbackPublisher,
  resolveFacebookPublisher,
  resolveInstagramPublisher,
  resolveThreadsPublisher,
}) {
  const context = {
    clientId: post.clientId,
    accountId: target.accountId,
    post,
    target,
    account,
  };

  if (target.platformId === 'facebook') {
    if (resolveFacebookPublisher) return resolveFacebookPublisher(context);
    if (account?.credentials?.pageId && account?.credentials?.pageAccessToken) {
      return createFacebookPublisher({
        pageId: account.credentials.pageId,
        pageAccessToken: account.credentials.pageAccessToken,
        graphVersion: process.env.META_GRAPH_VERSION || 'v25.0',
        graphBaseUrl: process.env.META_GRAPH_BASE_URL || 'https://graph.facebook.com',
      });
    }
    return fallbackPublisher;
  }
  if (target.platformId === 'instagram') {
    return resolveInstagramPublisher?.(context);
  }
  if (target.platformId === 'threads') {
    return resolveThreadsPublisher?.(context);
  }

  const error = new Error('不支援的發布平台。');
  error.status = 400;
  throw error;
}

async function publishTarget({
  postId,
  targetId,
  expectedPlatformId,
  source = 'manual',
  idempotencyKey = '',
  fallbackPublisher,
  resolveFacebookPublisher,
  resolveInstagramPublisher,
  resolveThreadsPublisher,
  repositories = getRepositories(),
}) {
  const posts = await repositories.posts.list();
  const postRaw = posts.find((item) => item.id === postId);
  if (!postRaw) {
    const error = new Error('找不到要發布的貼文。');
    error.status = 404;
    throw error;
  }

  const postCandidate = migrateLegacyPost(postRaw, postRaw.clientId || '');
  const targets = Array.isArray(postCandidate.targets) ? postCandidate.targets : [];
  let targetCandidate = targetId
    ? targets.find((item) => item.id === targetId)
    : null;
  if (targetId && !targetCandidate) {
    const error = new Error('找不到指定的發布目標。');
    error.status = 404;
    error.code = 'PUBLISH_TARGET_NOT_FOUND';
    throw error;
  }
  if (!targetCandidate && !targetId && targets.length === 1) {
    targetCandidate = targets[0];
  }
  if (!targetCandidate && !targetId && expectedPlatformId) {
    const platformTargets = targets.filter((item) => item.platformId === expectedPlatformId);
    if (platformTargets.length === 1) targetCandidate = platformTargets[0];
    if (platformTargets.length > 1) {
      const error = new Error('請指定要發布的多平台目標。');
      error.status = 409;
      error.code = 'PUBLISH_TARGET_AMBIGUOUS';
      throw error;
    }
  }
  if (!targetCandidate && !targetId && !expectedPlatformId && targets.length > 1) {
    const error = new Error('請指定要發布的多平台目標。');
    error.status = 409;
    error.code = 'PUBLISH_TARGET_AMBIGUOUS';
    throw error;
  }
  if (!targetCandidate || (expectedPlatformId && targetCandidate.platformId !== expectedPlatformId)) {
    const platformName = expectedPlatformId === 'facebook' ? ' Facebook' : '';
    const error = new Error(`找不到可發布的${platformName}目標。`);
    error.status = 400;
    throw error;
  }

  try {
    const approval = approvalGate(postCandidate, await getClientRaw(postCandidate.clientId));
    if (isPostIdea(postCandidate)) throw publishConflict('Ideas 尚未轉成草稿，不能直接發布。');
    if (!approval.allowed) {
      const error = publishConflict(approval.message);
      error.code = approval.code;
      throw error;
    }
    const attemptClaim = await beginPublishAttempt({
      postId,
      targetId: targetCandidate.id,
      source,
      idempotencyKey,
      repositories,
    });
    if (attemptClaim.replay) {
      return {
        externalId: attemptClaim.attempt.externalId,
        publishedAt: attemptClaim.attempt.finishedAt,
        targetId: attemptClaim.target.id,
        replayed: true,
      };
    }

    const post = attemptClaim.post;
    const target = attemptClaim.target;
    const attemptId = attemptClaim.attempt.id;
    await recordPublishAttemptEvent({
      postId: post.id,
      targetId: target.id,
      platformId: target.platformId,
      attempt: attemptClaim.attempt,
      eventType: 'started',
      occurredAt: attemptClaim.attempt.startedAt,
    });

    const client = await getClientRaw(post.clientId);
    const account = findAccount(client, target.accountId);
    const publisher = await resolvePublisher({
      post,
      target,
      account,
      fallbackPublisher,
      resolveFacebookPublisher,
      resolveInstagramPublisher,
      resolveThreadsPublisher,
    });

    if (!publisher?.configured) {
      throw unavailablePublisherError(target.platformId);
    }

    const copy = resolveTargetCopy(post, target);
    const publishPost = normalizePostCopy({
      ...post,
      facebook: target.contentType === 'reel' ? post.facebook : copy,
      reel: target.contentType === 'reel' ? copy : post.reel,
    });
    const mediaPaths = resolveTargetMedia(post, target);
    const resolvedContentType = (() => {
      if (target.platformId === 'instagram') {
        const contentType = String(target.contentType || '').trim();
        return !contentType || contentType === 'post' ? 'feed' : contentType;
      }
      return target.contentType || 'post';
    })();
    const publishOptions = {
      contentType: resolvedContentType,
      contentSettings: target.contentSettings || {},
    };
    if (target.platformId === 'facebook') {
      publishOptions.mediaFilePaths = mediaPaths.length
        ? mediaFilePathsFromWebPaths(mediaPaths)
        : resolvePostMediaPaths(post);
      if (!publishOptions.mediaFilePaths.length && mediaPaths.length) {
        publishOptions.mediaBuffers = await mediaBuffersFromWebPaths(mediaPaths);
      }
    } else {
      publishOptions.mediaWebPaths = mediaPaths;
    }
    const validation = await validateTargetFormat({
      platformId: target.platformId,
      contentType: resolvedContentType,
      copy,
      mediaPaths,
      targetId: target.id,
      uploadsDirectory: directories.uploads,
    });
    if (!validation.valid) throw formatValidationError(validation);
    const result = await publisher.publish(publishPost, publishOptions);

    const publishedAt = new Date().toISOString();
    const finalization = await repositories.posts.mutate((records) => {
      const current = records.find((item) => item.id === post.id);
      if (!current) return { kind: 'reconciliation_required', reason: 'POST_MISSING' };
      const migrated = migrateLegacyPost(current, current.clientId || post.clientId);
      Object.assign(current, migrated);
      if (Number(migrated.version || 1) !== Number(post.version || 1)) {
        return { kind: 'reconciliation_required', reason: 'POST_VERSION_CONFLICT' };
      }
      const currentTarget = (current.targets || []).find((item) => item.id === target.id);
      if (!currentTarget) return { kind: 'reconciliation_required', reason: 'TARGET_MISSING' };
      Object.assign(currentTarget, {
        status: 'published',
        publishedAt,
        externalId: result.externalId,
        lastError: null,
      });
      updatePublishAttempt(currentTarget, attemptId, {
        status: 'succeeded',
        finishedAt: publishedAt,
        externalId: result.externalId,
      });
      current.status = summarizePostStatus(current.targets);
      bumpPostVersion(current);
      current.publishedAt = publishedAt;
      if (target.platformId === 'facebook') {
        current.facebookPostId = result.externalId;
      }
      return { kind: 'updated', post: current };
    });

    if (finalization?.kind === 'reconciliation_required') {
      throw remotePublishReconciliationError(result.externalId, finalization.reason);
    }
    const finalizedPost = finalization?.post;
    if (finalizedPost) await appendPostVersion({ post: finalizedPost, source: 'publish' });

    const firstComment = target.delivery?.firstComment;
    let firstCommentResult = null;
    if (target.platformId === 'instagram' && firstComment?.text) {
      try {
        firstCommentResult = await publishFirstComment({
          postId: post.id,
          targetId: target.id,
          text: firstComment.text,
          publisher,
          repositories,
        });
      } catch (error) {
        firstCommentResult = {
          targetId: target.id,
          firstComment: error.firstComment || {
            status: 'failed',
            text: firstComment.text,
            lastError: firstCommentFailure(error),
          },
        };
      }
    }

    await recordPublishAttemptEvent({
      postId: post.id,
      targetId: target.id,
      platformId: target.platformId,
      attempt: {
        ...attemptClaim.attempt,
        status: 'succeeded',
        finishedAt: publishedAt,
        externalId: result.externalId,
      },
      eventType: 'succeeded',
      occurredAt: publishedAt,
    });

    return { ...result, publishedAt, targetId: target.id, ...(firstCommentResult ? { firstComment: firstCommentResult.firstComment } : {}) };
  } catch (error) {
    if (error && typeof error === 'object') error.publishTargetId = targetCandidate.id;
    throw error;
  }
}

async function markTargetPublishFailure(postId, targetId, error, repositories = getRepositories()) {
  if (!postId || !targetId) return;
  const failure = await repositories.posts.mutate((records) => {
    const current = records.find((item) => item.id === postId);
    if (!current) return;
    const migrated = migrateLegacyPost(current, current.clientId || '');
    Object.assign(current, migrated);
    const currentTarget = (current.targets || []).find((item) => item.id === targetId);
    const reconciliationRequired = error?.code === 'REMOTE_PUBLISH_RECONCILIATION_REQUIRED';
    if (!currentTarget || (!reconciliationRequired && currentTarget.status !== 'publishing')) return null;
    const lastError = serializePublishError(error);
    currentTarget.status = 'failed';
    if (reconciliationRequired && error.externalId) currentTarget.externalId = error.externalId;
    currentTarget.lastError = lastError;
    updatePublishAttempt(currentTarget, currentTarget.lastAttemptId, {
      status: 'failed',
      finishedAt: lastError.at,
      error: lastError,
    });
    delete currentTarget.nextAttemptAt;
    current.status = summarizePostStatus(current.targets);
    bumpPostVersion(current);
    return {
      postId: current.id,
      targetId: currentTarget.id,
      platformId: currentTarget.platformId,
      attempt: currentTarget.publishAttempts?.find((item) => item.id === currentTarget.lastAttemptId),
      occurredAt: lastError.at,
    };
  });
  if (failure?.attempt) {
    await recordPublishAttemptEvent({
      ...failure,
      eventType: 'failed',
    });
  }
}

export function createPublishRouter({
  facebookPublisher,
  resolveFacebookPublisher,
  resolveInstagramPublisher,
  resolveThreadsPublisher,
  getClient = getClientRaw,
  repositories = getRepositories(),
} = {}) {
  const router = Router();

  router.post('/publish/target', async (request, response) => {
    const postId = request.body?.postId;
    const targetId = request.body?.targetId;
    const source = String(request.body?.source || 'manual');
    const idempotencyKey = String(request.get('Idempotency-Key') || request.body?.idempotencyKey || '').trim();
    try {
      const result = await publishTarget({
        postId,
        targetId,
        source,
        idempotencyKey,
        fallbackPublisher: facebookPublisher,
        resolveFacebookPublisher,
        resolveInstagramPublisher,
        resolveThreadsPublisher,
        repositories,
      });
      response.json(result);
    } catch (error) {
      await markTargetPublishFailure(postId, error.publishTargetId || targetId, error, repositories).catch(() => {});
      if (error.status === 404 || error.status === 400 || error.status === 409 || error.status === 503) {
        return response.status(error.status).json({ error: error.message });
      }
      const status = isPublicMediaConfigurationError(error)
        ? 400
        : error instanceof FacebookPublishError
        || error instanceof InstagramPublishError
        || error instanceof ThreadsPublishError
          ? 502
          : 500;
      response.status(status).json({ error: error.message || '發布失敗。' });
    }
  });

  router.post('/publish/facebook', async (request, response) => {
    const postId = request.body?.postId;
    const targetId = request.body?.targetId;
    const source = String(request.body?.source || 'manual');
    const idempotencyKey = String(request.get('Idempotency-Key') || request.body?.idempotencyKey || '').trim();
    try {
      const result = await publishTarget({
        postId,
        targetId,
        expectedPlatformId: 'facebook',
        source,
        idempotencyKey,
        fallbackPublisher: facebookPublisher,
        resolveFacebookPublisher,
        resolveInstagramPublisher,
        resolveThreadsPublisher,
        repositories,
      });
      response.json(result);
    } catch (error) {
      await markTargetPublishFailure(postId, error.publishTargetId || targetId, error, repositories).catch(() => {});
      if (error.status === 404 || error.status === 400 || error.status === 409 || error.status === 503) {
        return response.status(error.status).json({ error: error.message });
      }
      const status = error instanceof FacebookPublishError ? 502 : 500;
      response.status(status).json({ error: error.message || 'Facebook 發布失敗。' });
    }
  });

  router.post('/publish/target/first-comment', async (request, response) => {
    const postId = String(request.body?.postId || '').trim();
    const targetId = String(request.body?.targetId || '').trim();
    try {
      const posts = await repositories.posts.list();
      const post = posts.find((item) => item.id === postId);
      if (!post) return response.status(404).json({ error: '找不到要留言的貼文。' });
      const normalized = migrateLegacyPost(post, post.clientId || '');
      const target = normalized.targets?.find((item) => item.id === targetId);
      if (!target) return response.status(404).json({ error: '找不到要留言的平台目標。' });
      const client = await getClient(normalized.clientId);
      const account = findAccount(client, target.accountId);
      const publisher = await resolvePublisher({
        post: normalized,
        target,
        account,
        fallbackPublisher: facebookPublisher,
        resolveFacebookPublisher,
        resolveInstagramPublisher,
        resolveThreadsPublisher,
      });
      const result = await publishFirstComment({
        postId,
        targetId,
        text: request.body?.text,
        publisher,
        repositories,
      });
      return response.json(result);
    } catch (error) {
      if ([400, 404, 409, 503].includes(error.status)) {
        return response.status(error.status).json({ error: error.message, code: error.code, firstComment: error.firstComment });
      }
      const status = error.status || (error instanceof InstagramPublishError ? 502 : 500);
      return response.status(status).json({ error: error.message || '首則留言失敗。', code: error.code, firstComment: error.firstComment });
    }
  });

  return router;
}
