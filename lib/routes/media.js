...
export function createMediaRouter({
  ...
  router.get('/media', async (request, response) => {
    ...
    await backfillLocalAssetChecksums({
      clientId,
      mediaStorage: storageOf(),
      repositories,
    });
  ...
  router.post('/media/upload-session', async (request, response) => {
    try {
      ...
      if (checksumSha256) {
        const existing = await findReadyMediaAssetByChecksum({ clientId, checksum: checksumSha256 }, repositories);
        if (existing) return response.json({ reused: true, asset: existing });
      }
