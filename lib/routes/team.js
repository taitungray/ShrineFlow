...
export function createTeamRouter({ repositories = getRepositories(), authService } = {}) {
  const router = Router();
  ...
  router.post('/invitations', async (request, response) => {
    try {
      ...
      response.status(201).json({
        ...
        invitationUrl: `${origin}/?invite=${encodeURIComponent(result.token)}`,
      });
    } catch (error) {
      sendError(response, error);
    }
    ...
    try {
      ...
      if (status === 'suspended') {
        const memberships = await repositories.memberships.query({
          filters: { userId: user.uid, status: 'active' },
        });
        for (const membership of memberships.filter((item) => item.role === 'owner')) {
      ...
      const events = (await repositories.auditEvents.list())
        .filter((event) => (
          ...
          && (event.clientId
            ...
            : (request.actor?.legacy || request.actor?.systemRole === 'owner'))
