...
export function createSettingsRouter({ onReloadSettings }) {
  ...
  router.post('/settings/rotate-secrets', async (request, response) => {
    ...
    try {
      clientBackupPath = await createRotationBackup(jsonFiles.clients);
      envBackupPath = await createRotationBackup(environmentFilePath);
      ...
      const rotatedClients = await rotateClientSecrets(currentMasterKey, newMasterKey);
      ...
      for (const key of ROTATION_SECRET_ENV_KEYS) {
        if (rotatedEnv.plainEnv[key] !== undefined) process.env[key] = rotatedEnv.plainEnv[key];
      }
