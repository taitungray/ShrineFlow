...
export async function inspectSystemHealth({
  ...
  const cloudRuntime = repositories.backend === 'firestore' && getMediaStorage().backend === 'r2';
  const jsonEntries = cloudRuntime
    ? await Promise.all(Object.keys(jsonFiles).filter((name) => repositories[name]).map(async (name) => {
      const value = await repositories[name].list();
      return [name, {
        status: 'ok',
        provider: 'firestore',
        bytes: null,
        itemCount: countStoredItems(name, value),
        maxItems: COLLECTION_STORAGE_POLICY[name]?.maxItems || null,
        maxBytes: null,
        itemLimitReached: false,
        byteLimitReached: false,
        usage: { itemRatio: null, byteRatio: null, status: 'ok', warningReasons: [] },
      }];
    }))
    : await Promise.all(Object.entries(jsonFiles).map(async ([name, filePath]) => [
      name,
 ...
 const jsonStatus = Object.fromEntries(jsonEntries);
  ...
  return {
    ...
    storage: {
      backend: cloudRuntime ? 'firestore+r2' : 'local-json+filesystem',
      jsonFiles: {
