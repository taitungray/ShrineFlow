...
export function createAiService() {
  ...
  async function analyzeContentPerformance({ clientName = '', platform = 'all', posts = [] } = {}) {
    reloadConfig();
    if (!gemini) {
      const error = new Error('尚未設定 GEMINI_API_KEY。請先在「⚙️ 設定」填入 API Key。');
      error.status = 503;
      throw error;
    }

    const postSummaries = posts.slice(0, 20).map((p, index) => {
      const metricsText = Object.entries(p.metrics || {})
        .map(([key, val]) => `${key}: ${val}`)
        .join(', ');
      return `[貼文 ${index + 1}]
標題/主題：${p.title || '未命名'}
平台：${p.platformId || platform}
發布時間：${p.publishedAt || '未知'}
文案摘要：${(p.copy || '').slice(0, 160)}
成效指標：${metricsText || '無外部指標紀錄'}`;
    }).join('\n\n');

    const prompt = `品牌名稱：${clientName || '社群品牌'}
目標平台：${platform === 'all' ? '全部社群平台' : platform}
分析樣本：共 ${posts.length} 篇已發布貼文

貼文清單與成效紀錄：
${postSummaries || '目前尚無詳細貼文成效，請根據常見的社群經營最佳實踐與宮廟/文創題材給予起始建議。'}

請分析這些內容的受眾反應、主題表現與互動亮點，並產出精闢的成效總結與下一步發文靈感。`;
    ...
    try {
      const { result } = await generateWithFallback({
        ...
        generate: (model) => gemini.models.generateContent({
          ...
          config: {
            systemInstruction: '你是專業的社群經營顧問與內容數據分析師。請分析貼文數據與題材，以繁體中文輸出結構化 JSON，不要輸出 Markdown 標籤或無關文字。',
            responseMimeType: 'application/json',
            responseJsonSchema: {
              type: 'object',
              properties: {
                summary: { type: 'string', description: '整體表現與受眾偏好重點總結（2-3句）' },
                topThemes: {
                  type: 'array',
                  items: {
                    type: 'object',
                ...
                nextPostIdeas: {
                  ...
                  items: {
                    ...
                    required: ['title', 'topic', 'format', 'reason', 'prompt'],
                  },
                },
              },
              required: ['summary', 'topThemes', 'actionableTips', 'nextPostIdeas'],
              additionalProperties: false,
            },
          },
        }),
