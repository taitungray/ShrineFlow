...
import { normalizeContentStage } from './post-lifecycle.js';
import {
...
function safeTarget(target = {}) {
  ...
  return {
    ...
    delivery: {
      firstComment: {
        status: String(firstComment.status || 'disabled'),
        text: String(firstComment.text || '').slice(0, 2000),
        externalId: firstComment.externalId || null,
      },
    },
  };
}
...
export function snapshotPostContent(post = {}) {
  return {
    ...
    contentStage: normalizeContentStage(post.contentStage),
...
function cloudRepository() {
  try {
    ...
    return repositories.backend === 'firestore' ? repositories.postVersions : null;
  } catch {
    return null;
  }
}
