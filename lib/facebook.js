...
export function createFacebookPublisher({
  ...
  async function recoverPageSession() {
    ...
    try {
      const me = await nodeRequest('me', { method: 'GET', query: { fields: 'id,name' } });
      if (me?.id && String(me.id) !== String(activePageId)) {
        activePageId = String(me.id);
        return { id: activePageId, name: me.name || '' };
      }
    } catch {
      // Token cannot identify a different page.
    }
    return null;
  }
  ...
  async function publish(post, options = {}) {
    try {
      ...
      return publishOnce(post, options);
    }
  }
