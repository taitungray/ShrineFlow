...
function normalizeMetadata(raw = {}) {
  return {
    ...
    needsReply: raw.needsReply === true || raw.needsReply === 'true',
...
function isProtected(record) {
  return Boolean(record.note || record.tags?.length || record.needsReply);
}
...
function cloudRepository() {
  try {
    ...
    return repositories.backend === 'firestore' ? repositories.inboxMetadata : null;
  } catch {
    return null;
  }
}
...
function metadataFallback() {
  return { version: INBOX_METADATA_POLICY.version, items: {}, cursors: {}, syncHints: {} };
}
...
async function mutateMetadata(mutator) {
  ...
  return repository
    ...
    : mutateJson(jsonFiles.inboxMetadata, mutator, metadataFallback());
}

async function readMetadata() {
  const metadata = await (cloudRepository()?.list() || readJson(jsonFiles.inboxMetadata, metadataFallback()));
  return {
...
export async function updateInboxItemMetadata(identity, changes = {}) {
  ...
  return mutateMetadata((metadata) => {
    ...
    const next = normalizeMetadata({
      ...
      needsReply: changes.needsReply === undefined ? previous.needsReply : changes.needsReply,
...
export async function applyInboxItemMetadata(items, identity) {
  ...
  return (Array.isArray(items) ? items : []).map((item) => {
    ...
    return {
      ...
      needsReply: Boolean(record?.needsReply),
      tags: record?.tags || [],
      note: record?.note || '',
