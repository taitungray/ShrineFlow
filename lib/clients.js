...
export async function createClient({ name, notes = '' } = {}) {
  ...
  if (!trimmedName) throw new Error('客戶名稱為必填。');
...
function normalizeAccountInput(account = {}) {
  ...
  const id = String(account.id || '').trim()
    || (platformId === 'facebook' && pageId ? `facebook:${pageId}` : `${platformId}:${makeId()}`);

  const hasToken = Boolean(credentials.pageAccessToken || credentials.accessToken);
  const configured = account.configured !== undefined
    ? Boolean(account.configured)
    : (platformId === 'facebook' ? Boolean(pageId && hasToken) : hasToken);
