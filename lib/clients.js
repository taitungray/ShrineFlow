...
export async function getClientRaw(clientId, repositories = getRepositories()) {
...
export async function updateClientWorkflow(clientId, { approvalRequired } = {}, repositories = getRepositories()) {
  const updated = await mutateClients((clients) => {
    const client = clients.find((item) => item.id === clientId);
    if (!client) return null;
    if (approvalRequired !== undefined) client.approvalRequired = Boolean(approvalRequired);
    client.updatedAt = new Date().toISOString();
    return client;
  }, repositories);
  return updated ? maskClient(updated) : null;
...
export async function upsertAccount(clientId, accountInput) {
  ...
  const updated = await mutateClients((clients) => {
    ...
    let index = client.accounts.findIndex((item) => item.id === nextAccount.id);
    if (index < 0) {
      index = client.accounts.findIndex((item) => item.platformId === nextAccount.platformId);
    }
    if (index >= 0) {
      ...
      client.accounts = client.accounts.filter((item, itemIndex) => (
        itemIndex === index || item.platformId !== nextAccount.platformId
      ));
    } else {
...
export async function updateAccountQueue(clientId, accountId, queue, repositories = getRepositories()) {
  const updated = await mutateClients((clients) => {
    const client = clients.find((item) => item.id === clientId);
    const account = findAccount(client, accountId);
    if (!account) return null;
...
export async function updateClientCrisisPause(clientId, crisisPause, repositories = getRepositories()) {
  const updated = await mutateClients((clients) => {
    const client = clients.find((item) => item.id === clientId);
    if (!client) return null;
    client.crisisPause = crisisPause || null;
    client.updatedAt = new Date().toISOString();
    return client;
  }, repositories);
  return updated ? maskClient(updated) : null;
