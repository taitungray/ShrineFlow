...
export async function listClientsRaw() {
  const clients = await readJson(jsonFiles.clients, []);
  return clients.map((client) => decryptClientSecrets(client));
}

async function mutateClients(mutator) {
  return mutateJson(jsonFiles.clients, async (storedClients) => {
...
export async function rotateClientSecrets(currentMasterKey = '', newMasterKey = '') {
  return mutateJson(jsonFiles.clients, (storedClients) => {
