...
export function createFirebaseSessionAuthService({
  ...
  env = process.env,
  now = () => Date.now(),
  securityMonitor = createSecurityMonitor({ repositories, now }),
} = {}) {
  ...
  async function actorForUid(uid) {
    ...
    let memberships = await listMembershipsForUser(uid, repositories);
    if (user.systemRole === 'owner') {
      memberships = await ensureSystemOwnerMemberships(user, repositories);
    }
  ...
  async function authorizeIdentity(identity, inviteToken = '') {
    ...
    if (existing && (existingMemberships.length || existing.systemRole === 'owner')) {
      ...
      const memberships = user.systemRole === 'owner'
        ? await ensureSystemOwnerMemberships(user, repositories)
        : existingMemberships;
      return createActor({ user, memberships, systemRole: user.systemRole || '' });
    }
  ...
  async function reauthenticate({ idToken = '', actor = null } = {}) {
    if (!idToken) throw authError('A fresh Firebase ID token is required.', 400, 'REAUTH_ID_TOKEN_REQUIRED');
    const claims = await firebaseAuth.verifyIdToken(idToken, true);
    const uid = String(claims.uid || claims.sub || '').trim();
    if (!uid || (actor?.uid && uid !== actor.uid)) {
      throw authError('Fresh Firebase authentication does not match the current session.', 401, 'REAUTH_FAILED');
    }
    const maxAgeMs = Math.max(60 * 1000, Number(env.SHRINEFLOW_REAUTH_MAX_AGE_MS) || 5 * 60 * 1000);
    const authTimeMs = Number(claims.auth_time || 0) * 1000;
    if (!authTimeMs || now() - authTimeMs > maxAgeMs) {
      throw authError('A recently refreshed Firebase authentication is required.', 401, 'REAUTH_FRESH_TOKEN_REQUIRED');
    }
...
export function createEnvironmentAuthService({
  ...
  if (mode === 'firebase') {
    return createFirebaseSessionAuthService({
      ...
      repositories,
      env,
      securityMonitor: securityMonitor || createSecurityMonitor({ repositories }),
    });
  }
  ...
  return createAuthService({
    env: mode === 'disabled' ? {} : env,
    repositories,
    securityMonitor: securityMonitor || undefined,
  });
}
