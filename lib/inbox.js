...
const DEFAULT_FIELDS = {
  facebook: 'id,updated_time,participants,snippet,message_count,messages.limit(20){id,message,from,created_time}',
  instagram: 'id,updated_time,participants,snippet,message_count,messages.limit(20){id,message,from,created_time}',
...
function otherParticipant(item, ownerId = '') {
  const owner = String(ownerId || '');
  return (item.participants?.data || []).find((participant) => {
    const id = String(participant.id || '');
    return id && id !== owner;
  }) || null;
}

function normalizeMessages(item, ownerId = '') {
  const owner = String(ownerId || '');
  const raw = item.messages?.data || item.replies?.data || [];
  return raw
    .map((message) => {
      ...
      return {
        ...
        createdAt: message.created_time || message.timestamp || null,
        author: String(message.from?.name || message.username || '').trim(),
        fromPage: Boolean(fromId && fromId === owner),
      };
    })
...
function normalizeItem(platformId, item, type = 'conversation', ownerId = '') {
  const counterpart = otherParticipant(item, ownerId);
  const recipientId = item.recipient?.id
    || item.from?.id
    || counterpart?.id
    || null;
  const text = String(item.message || item.text || item.snippet || '').trim();
  const author = String(
    item.from?.name || item.username || item.sender?.name || counterpart?.name || counterpart?.username || '',
  ).trim();
  const messages = normalizeMessages(item, ownerId);
  const fallbackMessages = messages.length || !text
    ? messages
    : [{
      id: String(item.id || ''),
      text,
  ...
  return {
    ...
    replyCount: Number(item.message_count || item.replies?.data?.length || fallbackMessages.length || 0),
    recipientId,
    replyToId: item.reply_to_id || item.replied_to?.id || (type === 'reply' ? item.id : null),
  ...
  async function request(resource, query = {}, options = {}) {
    ...
    try {
      ...
      if (error?.code === 'PROVIDER_RATE_LIMIT_LOCAL') {
        throw new InboxApiError(error.message, {
          platformId,
          status: 429,
          code: error.code,
          retriable: true,
  ...
  return {
    ...
    async fetchRecent({ limit = 25, after = '' } = {}) {
      ...
      return {
        ...
        items: (payload.data || []).map((item) => normalizeItem(options.platformId, item, 'conversation', options.resourceId)),
        paging: payload.paging || null,
      };
    },
    async reply({ recipientId, text } = {}) {
      const recipient = String(recipientId || '').trim();
      const message = String(text || '').trim();
      if (!recipient || !message) throw new InboxApiError('回覆需要對方識別 ID 與文字內容。', { platformId: options.platformId, status: 400, category: 'validation' });
      const payload = await client.request(options.resourceId + '/messages', {}, {
        method: 'POST',
        body: {
      ...
      return { platformId: options.platformId, messageId: payload.message_id || payload.id || null, recipientId: recipient };
    },
  };
}
  ...
  return {
    ...
    async fetchRecent({ limit = 25 } = {}) {
      ...
      for (const thread of threadPayload.data || []) {
        ...
        const replies = (conversation.data || []).map((item) => normalizeItem('threads', item, 'reply', options.userId));
        if (replies.length) items.push(...replies);
        else items.push(normalizeItem('threads', thread, 'thread', options.userId));
      }
      return {
        platformId: 'threads',
        source: 'meta_graph_api',
    ...
    async reply({ replyToId, text } = {}) {
      const targetId = String(replyToId || '').trim();
      const message = String(text || '').trim();
      if (!targetId || !message) throw new InboxApiError('Threads 回覆需要 reply_to_id 與文字內容。', { platformId: 'threads', status: 400, category: 'validation' });
      const payload = await client.request('me/threads', {
        media_type: 'TEXT',
        text: message,
        reply_to_id: targetId,
      }, { method: 'POST' });
      return { platformId: 'threads', messageId: payload.id || null, replyToId: targetId };
    },
  };
}
